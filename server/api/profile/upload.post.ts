...
export default eventHandler(async (event) => {
  try {
    ...
    const { filePath, fileName, contentType, base64 } = body || {}
    if (!filePath || !base64) return { success: false, error: 'filePath ou base64 ausente' }
    ...
    // upload via admin client (service role)
    ...
    // atualiza funcionario por user_id
    ...
    console.error('[server/api/profile/upload] exception:', err)
